<template>
  ...
  <div class="main-content">
    <form @submit.prevent="handleUpdate" action="">
      <h3>Update Technology Page</h3>
      <input type="hidden" id="id" name="id" v-model="id" />

      <span><small>Technology updated by admintor</small></span>
...
export default {
  ...
  async mounted() {
    const technology = await axios.get(`/technologies/${this.id}`);
    this.tech = technology.data;
    console.log(this.tech);
  },
  methods: {
    async handleUpdate() {
      try {
        const tech = await axios.put(`/technologies/${this.id}`, this.tech);
        console.log(tech);
